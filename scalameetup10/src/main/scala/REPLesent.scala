...
case class REPLesent(
    width: Int = 0,
    height: Int = 0,
    source: String = "scalameetup10.txt",
  ...
  private case class Config(
    ...
    private def fill(s: String): String =
      if (s.isEmpty) s
      else {
  ...
  private case class Line(content: String,
                          length: Int,
                          private val style: Line.Style) {
  ...
  private object Line {
    ...
    private def ansi(line: String): (String, Int) = {
      ...
      val content: String = ansiEscape.replaceAllIn(
        line,
        m =>
          m.matched(1) match {
  ...
  private case class Build(content: IndexedSeq[Line],
                           size: Int,
                           maxLength: Int,
                           footer: Line)

  private case class Slide(content: IndexedSeq[Line],
                           builds: IndexedSeq[Int],
    ...
    def build(n: Int, footer: Line): Build =
      Build(content.take(builds(n)), content.size, maxLength, footer)
  }
  ...
  private case class Deck(slides: IndexedSeq[Slide]) {
    ...
    private def select(slide: Int = slideCursor,
                       build: Int = 0): Option[Build] = {
    ...
    def nextBuild: Option[Build] =
      select(build = buildCursor + 1) orElse jump(1)
    ...
    def runCode: Unit = {
      ...
      if (repl.isEmpty) {
        Console.err.print(
          s"No reference to REPL found. Please call with parameter intp=$$intp")
  ...
  private def parseSource(path: String): IndexedSeq[Slide] = {
    Try {
      ...
      val lines: Iterator[String] =
        (
          if (pathFile.isDirectory) {
            pathFile.list.sorted
              .filter(_.endsWith(".replesent"))
              .flatMap { name =>
                io.Source.fromFile(new File(pathFile, name)).getLines
              }
              .toIterator
          } else {
      ...
      case Failure(e) =>
        e.printStackTrace
        Console.err.print(
          s"Sorry, could not parse $path. Quick, say something funny before anyone notices!")
        IndexedSeq.empty
  ...
  private def parse(lines: Iterator[String]): IndexedSeq[Slide] = {
    ...
    object CodeHandler extends LineHandler {
      private val patterns: Seq[(String, Regex)] = {
        ...
        val typeSig: Regex = {
          val token: String => String = { limit =>
            s"[$$_]${limit}[A-Z][_$$A-Z0-9]${limit}[\\w$$]${limit}"
          }
          val prefix
            : String = s"""(?<=(?::)\\s{0,10}|\\btype ${token("{0,10}")}\\s{0,10}=\\s{0,10})"""
      ...
      def apply(line: String): (Line, Option[String]) = {
        ...
        val formatted = regex.replaceAllIn(
          line, { m =>
